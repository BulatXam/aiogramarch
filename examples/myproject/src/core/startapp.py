import asyncio

from aiogram import Router

from typing import List
from loguru import logger
from dotenv import load_dotenv
from pathlib import Path

from fastapi import APIRouter

from tortoise.contrib.fastapi import register_tortoise
from uvicorn import Config, Server  # ASGI для fastapi
from fastapi_admin.app import app as admin_app

from .admin.core import configure_admin

from .config import bot, dp, fastapi_app

from .database.core import database_init, database_connections_close


async def _start_bot(bot_base_router: Router) -> None:
    logger.info("Include bot base_router")
    dp.include_router(bot_base_router)

    logger.info("Start bot polling")

    await dp.start_polling(bot)


async def _start_site(
    fastapi_routers: List[APIRouter], tortoise_orm: dict
) -> None:
    register_tortoise(fastapi_app, config=tortoise_orm)

    for router in fastapi_routers:
        fastapi_app.include_router(router)
        logger.info(f"Include fastapi router: {router.prefix}")

    await configure_admin()

    fastapi_app.mount("/admin", admin_app)
    logger.info("Start site")

    current_loop = asyncio.get_event_loop()
    config = Config(app=fastapi_app, loop=current_loop)
    server = Server(config)
    await server.serve()


async def _start_app(
    bot_base_router: Router,
    fastapi_routers: List[APIRouter], 
    tortoise_orm: dict,
) -> None:
    await asyncio.gather(
        _start_site(fastapi_routers=fastapi_routers, tortoise_orm=tortoise_orm), 
        _start_bot(bot_base_router=bot_base_router)
    )


async def run(
    bot_base_router: Router,
    fastapi_routers: List[APIRouter], 
    tortoise_orm: dict,
) -> None:
    """
    Функция, работающая в корневой папке проекта в main.py (ЭТО ЖИЗНЕННО ВАЖНО!)

    """
    load_dotenv(Path.cwd() / "env.env")

    await database_init(tortoise_orm=tortoise_orm)

    try:
        await _start_app(
            bot_base_router=bot_base_router,
            fastapi_routers=fastapi_routers, 
            tortoise_orm=tortoise_orm
        )
    finally:
        await database_connections_close()
